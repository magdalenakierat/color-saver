...
export default function ColorCard({ colorObjects, editCard }) {
  ...
  return (
    colorObjects.map((colorObject) => {
      return (
      <article key={colorObject.id} className="card" style={{ backgroundColor: colorObject.colorCode }} onClick={()=>{
        navigator.clipboard.writeText(colorObject.colorCode).then(()=>{
          alert("#Hex Farbcode im Zwischenspeicher");
        });
        }}>
